...
function Model(props) {
  ...
  if (data.models) {
    ...
    var rows2 = array2.reduce(function (rows2, key, index) {
      return (index % 3 == 0 ? rows2.push([key]) : rows2[rows2.length - 1].push(key)) && rows2;
    }, []);
  }
      ...
      <div className="modelBody">
          ...
          <div className="homeCarsDiv">
            {data.links
              ? data.links.map((link, index) => (
                  <a  key={link.id || index} id="homeAnchorStyle" className="btn btnsize" href={link.link}>
                    {link.name}
                  </a>
                ))
              : null}
        ...
        <div className="modelMiddleDiv">
          <div className="modelDescriptionDiv">
            <p className="modelNumber_p roboto">
              {data.acceleration}
              <span className="modelSmall_Letter">s</span>
            </p>
            <p className="modelDescription_p roboto">
              Acceleration 0-62 mph (0-100 km/h) with Launch Control
            </p>
            <p className="modelNumber_p roboto">
              {data.kw}
              <span className="modelSmall_Letter">kW / </span>
              {data.ps}
              <span className="modelSmall_Letter">PS</span>
            </p>
            <p className="modelDescription_p roboto">
              Maximum power (kW)/Maximum power (PS) Details of the measuring method can be found at{" "}
              <a className="inputAnchor" href="/model">
                {" "}
            ...
            <p className="modelNumber_p roboto">
              {data.speed}
              <span className="modelSmall_Letter">mph</span>
            </p>
          ...
          >
            ...
            {data.models
              ? data.models.map((model, index) => {
                  <div key={index}>{createModel(model)}</div>;
                })
              : null}
            {/*Adding array[i] to CarouselDiv*/}
          </Carousel>
          ...
          {data.models ? (
            ...
            >
              {rows.map((row, rowIndex) => (
                <div key={`row-${rowIndex}`} className="modelInlineDiv modelInlineDiv_Padding">
          ...
          {data.models ? (
            ...
            >
              {rows2.map((row, rowIndex) => (
                <div key={`row2-${rowIndex}`} className="modelInlineDiv modelInlineDiv_Padding">
                  {row.map((object) => createModel(object))}
                </div>
              ))}
            </Carousel>
        ...
        <div className="model_4thDiv">
          <div className="modelCard">
            <div
              className="modelCardImageCar modelBackgroundimage"
              style={{ backgroundImage: "url(" + data.imageCard + ")" }}
            ></div>
            <div className="modelCard2">
              <p className="roboto modelStart_p modelDivTitle">Finance from Porsche.</p>
              <p className="roboto modelDescription_p14px modelMarginTop modelDivDescription">
                Driving a Porsche is an exciting and unique experience. With Porsche Financial
                Services you unlock that dream. Explore the finance options available to help you
                secure your new all models.
              </p>
              <div className="modelCardButton visibilitySetA">
                <button
                  id="modelMarginRigh33px modelMarginTop"
                  className="loginInput loginButton signupInput signupButton modelButton"
                >
          ...
          <div className="modelLastDiv">
            ...
            <p className="roboto modelDescription_p14px">
              ...
              {data.models
                ? data.models.map((model, index) => (
                    <span key={index}>
                      <span className="modelDescription_p500w">
                        {" "}
                        {model.title} Models (WLTP)*:{" "}
                      </span>
                      {model.emissions}
                    </span>
                  ))
